...
random.gamma2 <- function(n,alpha){
  ...
  x=rep(0,n)
  a=log(((alpha-1)/exp(1))^((alpha-1)/2))
  b=log(((alpha+1)/exp(1))^((alpha+1)/2))
  for (i in 1:n){
    inside=FALSE
    while (inside==FALSE){
      u1=sample(a) 
...
#Testing function 
alpha=300 #FUNKER IKKE FOR ALPHA>300......
...
$$
f_X(x_1,\dots,x_{K-1})=  \frac{\int_0^\infty v^{(\sum_{k=1}^{K}\alpha_i)-1}e^{-v}dv}{\prod_{k=1}^K\Gamma(\alpha_k)}\dot{} \prod_{k=1}^{K-1}x_k^{\alpha_k-1} \dot{} \left(1-\prod_{k=1}^{K-1}x_k\right)^{\alpha_K-1}
$$
