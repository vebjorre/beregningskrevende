...
ggplot() +
  ...
  labs(
    caption = expression("Histogram of sample from exponential distribution with "*lambda*"=3 compared to theoretical density.")
  )
```
...
#Normalised density function for g (for plot)
g <- function(x, a) {
  c <- c.g(a)
  out <- rep(0, length(x))
  left <- 0 < x & x < 1
...
ggplot() +
  ...
  xlim(0,5) +
    labs(
      caption = bquote("Histogram of sample from g-distribution with "~alpha*"=0.6 compared to theoretical density.")
    )
```

The histogram and density function coincide and that further indicates that the implementation is correct.
...
ggplot() +
  ...
  labs(
    caption = expression("Histogram of sample from standard normal distribution compared to theoretical density.")
  )
...
sample <- function(ab){ #This function sample from the distribution of log(x), using the inverse of the cumulative distribution. 
  u=log(runif(1,0,1))
  return (ab+u)
}
```
...
ggplot() +
  ...
  labs(
    caption = expression("Number of trials needed to generate 1000 samples from gamma distribution as a function of "*alpha)
  )
...
plot.gamma <- function(n,alphav,betav){
  ...
  ggplot() +
    ...
    xlim(0,5) +
    labs(
      caption = bquote("Histogram of sample from gamma distribution with " ~ alpha * "=" ~ .(alphav) ~ " and " ~ beta *"="* .(betav) ~ " compared to theoretical density.")
    )
}
...
theta_new_f_g <- function(theta){ #theta*new_f with prior beta(1,5)
  return (theta*new_f(theta)*(1-theta)^4)
}
```
```{r}
...
random.multinomial <- function(n){
  ...
  return(list(theta = x.out, tries = rejections+n))
}
```
To estimate the posterior mean of $\theta$ we use Monte-Carlo integration with M=10000 samples from $f(\theta|y)$. 

```{r}
sample.multinomial <- random.multinomial(10000)
cat("Estimated posterior mean:", mean(sample.multinomial), "\n")
```
...
```{r}
M=10000
average_theta_tries <- random.multinomial(M)$tries /M
cat("Our sampling algorithm need to generate on average", average_theta_tries, "random numbers, in order to obtain one sample. \n")
```

We would expect, on average, to generate $c$ samples before one i accepted, since the overall acceptance rate is $c^{-1}$. Since the propsal density is $\mathcal{U}(0, 1)$, we have $c\geq f(x)$. Hence, we can numerically calculate $c = \max_{\theta \in [0, 1]} f(\theta | \textbf{y})$.
...
importance_sampling <- function(n){ 
  theta <- random.multinomial(n)$theta #samples from f(theta|y) with the rejection sampling
...
ggplot() +
    ...
    labs(
      caption = expression("Histogram of sample from the posterior distribution compared to its theoretical density.")
    )
