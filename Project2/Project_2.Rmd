...
#Single site MCMC with rw proposal
mcmc_RW <- function(d, ntimes = 1000){ #x1=t1, x2=lam0, x3=lam1, x4=beta, x5=y0
  x <- matrix(nrow=ntimes, ncol=5)
  x[1,1] <- runif(1,t0,t2) #Init t1
  x[1,2:4] <- runif(3,0,1) #Init lam0,lam1,beta
  x[1,5] <- sum(date<x[1,1]) #Init y0|t1
  for(i in 2:ntimes)
  {
    for (j in 1:4){
      y <- max(0,runif(1,x[i-1,j]-d[j],x[i-1,j]+d[j])) #Random walk proposal >0
      temp <- x[i-1,]
      temp[j] <- y
      alpha <- min(1,exp(fullcond(temp,j) - fullcond(x[i-1,],j)), na.rm=TRUE) #Acceptance probability
      if (runif(1)<alpha){ #Acceptance step
        if (j==1){ #Constraint on t1
          if (t0 <= y && y <= t2){ 
            x[i,j] = y
          }
          else{
    ...
    x[i,5] <- sum(date<=x[i,1]) #Update y0|t1
  }
  return(x)
}
```
...
par(mfrow=c(2,2))
truehist(x[-c(1:1000),1])
truehist(x[-c(1:1000),2])
truehist(x[-c(1:1000),3])
truehist(x[-c(1:1000),4])
...
## 7) 
```{r}
#Joint full conditional for lambda0 and lambda1 on logscale
fullcond.b1 <- function(x){ #x1=t1, x2=lam0, x3=lam1, x4=beta, x5=y0
  return (-5*log(x[4]) + (x[5]+1)*log(x[2]) + (189-x[5]+1)*log(x[3]) - (1+x[2]+x[3])/x[4] + x[1]*(x[3]-x[2]) + x[2]*t0 - x[3]*t2 )
}

#Block MH with rw proposal
