...
#Omit the first 1000 samples to get good estimates.
...
```{r}
#Histogram for theta
truehist(f_gibbs[,1],xlim = c(0,5),xlab="Marginal for theta")
```

The histogram shows that the marginal posterior of $\theta$ are close to a Gamma distribution, which is the same kind of distribution $\theta$ was generated from, $\pi(\theta|\mathbf{y},\boldsymbol{\eta})$. 
...
plot(theta,pi_theta_k, xlab="", ylab="", 
     sub="Marginal posterior distribution for the hyperparameter theta")
lines(s)

```

The figur above shows the marginal posterior for the hyperparameter $\pi(\theta|\mathbf{y})$ and the points indicate the density at the chosen point $\theta^1,\dots, \theta^K$. The marginal posterior distribution are close to identically with the MCMC estimate we obtained in point 2). This is not suprising since in the case with Gaussian likelihood we can compute the full conditional $\pi(\boldsymbol{\eta}|\theta,\mathbf{y})$ exactly, instead of approximating it.   
...
plot(eta_grid,pi_eta_10, 
     xlab="Marginal posterior distribution for the smooth effect eta_10", ylab="")
...
posterior for the smooth efftct for $i=10$, $\pi(\eta_{10}|\mathbf{y})$ found by the inla scheme and the last figure shows the estimation obtained via Gibbs sampling. The distributions are almost identically. Since the marginal distribution $\pi(\boldsymbol{\eta}|\theta,\mathbf{y})$ can be easily found, the only approximation error for $\pi(\eta_i|\mathbf{y})$ with inla comes from the numerical integration. 
...
plot(result$marginals.hyperpar$`Precision for t`,xlim=c(0,6), xlab="", ylab = "", 
     sub=" Posterior distribution for the hyperparameter theta")

#Plot the marginal posterior for eta_10
plot(result$marginals.random$t$index.10, xlim=c(-2,2), xlab = "", ylab="", 
     sub="Marginal posterior distribution for the smooth effect eta_10")
```

The result from inla is close to identically with the result obtained with Gibbs sampling and the inla scheme. 
