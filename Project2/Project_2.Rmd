...
## 5)
We set the tuning parameter $\sigma$ to 1, such that the proposal distribution for $t_1$ is the standard normal, and do only 2000 steps of our algorithm to evaluate its properties in the early steps.
```{r}
#tuning parameter
sd <- 1
...
```
In the figure the points display the real data and the red lines show the posterior estimates of the process.  The plot shows that the estimated means represent the data very well. 
...
```
For $\sigma=5$ everything look a lot better, even when we plot only the first 2000 steps. The burn-in period seem to be just around 10 steps and the autocorrelation is better than with $\sigma=1$ as was the very first try.
```{r}
par(mfrow=c(1,2))
plot(x5[1:2000,1], type='l', xlab="i", ylab="t1")
acf(x5[-burnin,1], main="t1")
```
One could think that increasing $\sigma$ even more would only be positive, but if we look at the situation with $\sigma=20$ we see that the trace plot is fluctuating as much as we want. Most steps seem to long, but a lot of steps are rejected. This is because many proposed steps are too long and move far away from the optimal value. We also see that the autocorrelation is worse than for $\sigma=5$. Hence from the four different values for $\sigma$ we tried, $\sigma=5$ is the best one. Note that there are probably other values that are even better, but $\sigma=5$ is clearly sufficiently good to use.

Now we plot histograms approximating the posterior marginal distribution of $\lambda_0$ from each of the three situations, after omitting the 10000 first steps.
```{r}
burnin=c(1:10000)
par(mfrow=c(2,2))
...
```
We see that the three histograms are close to identical and it seems safe to assume that they have the same limiting distribution.

## 7) 
Instead of single site updates we now do want to do two block proposals. In the first block we keep $\beta$ unchanged and start by generating $\tilde t_1$ from a normal distribution with the current value of $t_1$ as mean and standard deviation $\sigma_1$. We then generate $\lambda_0$ and $\lambda_1$ from their joint full conditional given $\tilde t_1$, i.e. $f(\lambda_0, \lambda_1 | x, \tilde t_1, \beta)$. We observe that this joint density is simply the product of their marginal full conditionals. Thus they are independent and can be generated from their separate marginals $f(\lambda_i | x, \tilde t_1, \beta)$. Since we now update three values at the same time it is reasonable to just use the joint density function of all the parameters in the acceptance probability. Thus, 
$$
\alpha = \min\{1,\frac{f(\tilde\theta|x)}{f(\theta|x)}\},
$$
where $\tilde\theta=(\tilde t_1, \tilde\lambda_0, \tilde\lambda_1, \beta)$.
The second block is identical to the first except keeping $t_1$ constant and instead proposing $\beta$ from the normal distribution with $\sigma_2$ as standard deviation. We use the same initial values as in single site algorithm.
...
lines(x=c(t1hat2,t2), y=c((t1hat2-t0)*l0hat2, (t1hat2-t0)*l0hat2+(t2-t1hat2)*l1hat2), col="red", lw=2)
```
From the print-out we quickly see that these values are the almost the same as those from the single site algorithm, and the two plots are naturally almost indistinguishable.
...
```
The print-out shows that this is practically zero, as was expected since their joint full conditional was the product of the two marginal full conditionals.
...
#The block Gibbs sampling algorithm for the posterior distribution for theta and eta given y
